...
vim.keymap.set(
  ...
  '<Cmd>call jobstart("tmux send-keys -t bottom-left node Space '
    .. string.gsub(string.gsub(vim.api.nvim_buf_get_name(0), '.ts', '.js'), '/src/', '/dist/')
    .. ' Enter")<CR>',
...
-- Toggle comment with Ctrl+¨ (sends <C-]> on Norwegian keyboard)
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        -- Only format on save for these filetypes (empty = disabled)
